...
## JIRA Explorer и формулы метрик

Страница `/ui/jira` показывает список доступных полей из выборки задач (JQL из `JIRA_JQL_FILTER`). Для формирования формул в кастомных метриках доступна переменная `jira_issues` — список объектов задач в формате JIRA REST (`[{"key": "ABC-1", "fields": {...}}, ...]`).

Доступные вспомогательные функции в выражениях:
- `avg(seq)` — среднее значение или `None`
- `count_if_equal(seq, value)` — количество элементов равных `value`
- стандартные: `len`, `sum`, `min`, `max`, `round`

Пример выражения (среднее по полю Story Points):
```
avg([i['fields'].get('customfield_story_points', 0) for i in jira_issues])
```

Допустимые имена переменных цикла в list comprehension: `i, j, k, x`.

Если JIRA отключена (`JIRA_ENABLED=false`), `jira_issues` будет пустым.

### Перезагрузка динамических метрик и кэш JIRA

При добавлении/изменении `custom_metrics.json` метрики автоматически регистрируются при старте UI. Если файл меняется вне UI формы, можно вручную вызвать функцию:

```python
from scrum_kitchen_metrics.ui.router import reload_dynamic_metrics
reload_dynamic_metrics()
```

Это безопасно (добавляются только новые ключи, дубликаты игнорируются).

Для снижения количества обращений к JIRA введён простой TTL-кэш (5 секунд) на уровне `_fetch_jira_issues` внутри `ui/router.py`. Если несколько динамических метрик используют `jira_issues`, повторные вызовы в течение короткого окна не обращаются к API повторно.

### Ограничения `_safe_eval`

Дополнительная защита парсинга выражений:
* Максимальная длина выражения: 500 символов.
* Блокируются токены: `__`, `import`, `lambda`, `eval`, `exec`.
* Лимит глубины круглых скобок: 25.
...
### Кастомные метрики по changelog

Через UI (раздел *Metrics → JIRA Changelog Metrics*) можно задать собственные метрики на основе переходов между статусами. Для каждой метрики указывается:

- `JQL` — срез задач;
- набор входных статусов (*From statuses*) и целевых статусов (*To statuses*);
- функция агрегации (`avg`, `median`, `p90`, `max`, `min`, `sum` и др.);
- единица измерения (`days` или `hours`), лимит выборки и описание.

Конфигурация хранится в `custom_jira_changelog_metrics.json`. Для каждого определения создаётся динамическая метрика, использующая сервис `JiraChangelogMetricsService`, который загружает changelog задач, вычисляет длительность между указанными статусами и возвращает агрегированное значение вместе с детализированной расшифровкой. Таким способом можно настроить lead time, cycle time и любые пользовательские KPI без правки Python-кода.
